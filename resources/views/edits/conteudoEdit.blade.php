...
<script type="text/javascript">
    $(document).ready(function(){
        ...
        $.get("{{route('knowledge.show', Auth::user()->user_id)}}", function(data){
            ...
            $.each(data, function(i, linha) {
                 option+= "<option value='"+linha.mentor_id+"'"+(({{$conteudo->fk_content_knowledge}} == linha.mentor_id) ? ' selected' : '')+">"+linha.assunto+"</option>";
            });
  ...
  <div class="tab-content">
    <div id="conteudo" class="tab-pane fade in active">
        <br/>
        <br/>
        <form action="{{ route('content.update', $conteudo->content_id)}}" method="POST" class="form-horizontal">
        @method('PATCH')
            @csrf
            ...
            <div class="form-group{{ $errors->has('content_title') ? ' has-error' : '' }}">
                ...
                <div class="col-sm-10">
                    <input class="form-control" type="text" name="content_title" id="titulo" value="{{$conteudo->content_title}}"/>
            ...
            <div class="form-group{{ $errors->has('content_content') ? ' has-error' : '' }}">
                ...
                <div class="col-sm-10">
                    <textarea name="content_content" class="form-control" id="editor">
